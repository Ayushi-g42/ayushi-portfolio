...
export function Experience() {
  return (
    <section id="experience" className="section bg-dark-surface/30">
            ...
            {experiences.map((exp, idx) => (
              <motion.div key={exp.id} variants={fadeUpVariant} className="flex gap-6 relative">
                ...
                <div className="relative flex-shrink-0 mt-1">
                  ...
                  {exp.current && (
                    <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-emerald-400 border-2 border-dark-bg animate-pulse-slow" />
                  )}
                </div>

                {/* Content */}
                <div className="flex-1 card-glow p-6 rounded-3xl border border-dark-border bg-dark-surface hover:bg-dark-surface2 transition-all group">
                  {/* Header */}
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {exp.current && (
                          <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-400/10 text-emerald-400 border border-emerald-400/20">
                            Current
                          </span>
                        )}
                      </div>
                      <p className="text-accent-400 font-semibold text-sm">{exp.company}</p>
                    </div>
                  ...
                  <ul className="flex flex-col gap-2.5 mb-5">
                    {exp.achievements.map((ach, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-sm text-text-muted">
                        <CheckCircle2 size={15} className="text-accent-400 flex-shrink-0 mt-0.5" />
                        <span>{ach}</span>
                      </li>
                    ))}
                  </ul>
